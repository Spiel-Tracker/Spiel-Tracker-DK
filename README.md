...
diff --git a/Neuer Ordner/index.html b/Neuer Ordner/index.html
index 2437a9333864103c9517d07fc1e744900f04f842..b3481109e83fbb3149725b7e5893dbbe213740c2 100644
--- a/Neuer Ordner/index.html	
+++ b/Neuer Ordner/index.html	
@@ -1,1270 +1,1964 @@
-<!DOCTYPE html>
-<html lang="de">
-<head>
...
-    }
-
-    /* ===== Mobile: alles etwas nach unten (Statusbar/Battery überdeckt sonst) ===== */
...
-          12px;
-      }
-      /* Topbar noch etwas höher, damit iOS Statusbar sicher frei bleibt */
-      .topbar{
-        padding:
...
-      }
-
-      /* Timer auf Mobile im Match noch größer */
...
-<div class="topbar">
-  <div class="row">
-    <div class="app-title">⚽ Spiel-Tracker</div>
-
...
-      <div class="sep"></div>
-
-      <h3>Startelf/Bank auswählen</h3>
-      <p class="hint">1× → Startelf (grün) · 2× → Bank (blau) · 0× → nicht dabei (grau)</p>
-
-      <div id="setupGrid" class="setup-grid" style="margin-top:10px"></div>
-
-      <div class="toolbar" style="margin-top:12px;">
-        <button id="startBtn" type="button" class="success" disabled>▶️ Spiel starten</button>
-        <button id="resetSetupBtn" type="button" class="ghost">Alle zurücksetzen</button>
-        <button id="resetRosterBtn" type="button" class="ghost">Kader Standard</button>
-      </div>
-
-      <div class="sep"></div>
-      <h3>Kader ändern</h3>
-      <p class="hint">Selten nötig: hier kannst du Neuzugänge hinzufügen oder Abgänge löschen.</p>
-
-      <div class="toolbar" style="gap:10px;align-items:flex-end">
-        <div style="flex:1;min-width:220px">
-          <label class="hint" for="newPlayerName">Spieler hinzufügen</label><br>
-          <input id="newPlayerName" type="text" placeholder="Name eingeben" style="margin-top:6px;width:100%"/>
-        </div>
-        <button id="addPlayerBtn" type="button" class="success">+ Hinzufügen</button>
-      </div>
-
-      <p class="hint">Löschen im Setup: ✕ oben rechts (nur wenn Spieler grau ist). Kader wird offline gespeichert.</p>
-    </div>
-  </section>
-
-  <!-- MATCH -->
...
-
-      <div class="grid2">
-        <button id="pauseBtn" type="button" class="secondary">⏸️ Pause</button>
-        <button id="resumeBtn" type="button" class="success" hidden>▶️ Weiter</button>
-      </div>
-
-      <div class="toolbar" style="margin-top:10px;justify-content:space-between;">
-        <button id="swapWizardBtn" type="button" class="ghost">🔁 Wechsel-Assistent</button>
-        <div class="toolbar">
-          <button id="goalBtn" type="button" class="ghost">⚽ Tor (markiert)</button>
-          <button id="ownGoalBtn" type="button" class="ghost danger">⚠️ Gegentor</button>
-          <button id="endBtn" type="button" class="danger">⏹️ Spiel beenden</button>
-        </div>
-      </div>
-
-      <div class="sep"></div>
...
-        <button id="toggleEventsBtn" type="button" class="ghost">📋 Anzeigen</button>
-      </div>
-      <div id="eventsWrap" hidden style="margin-top:10px">
-        <div id="eventLog" class="timeline"></div>
-      </div>
-
-      <p class="hint">Spielerkarte antippen = markieren. Wechsel nur über den Wechsel-Assistenten.</p>
-    </div>
-
-    <div class="card">
-      <h3>Spieler</h3>
-      <div class="list" id="playerList"></div>
-      <p class="hint">Nur Feldspieler können markiert werden (für Tor/Assist). Sortierung: Feld oben, Bank unten (A–Z).</p>
-    </div>
-
...
-  <div class="row">
-    <button id="bbSwap" type="button" class="ghost">🔁 Wechsel</button>
-    <button id="bbGoal" type="button" class="ghost">⚽ Tor</button>
-    <button id="bbEnd" type="button" class="danger">⏹️ Ende</button>
-  </div>
-</div>
-
-<!-- Assist Modal -->
...
-  <div class="box">
-    <h4>Assist auswählen (optional)</h4>
...
-    <div class="sep"></div>
-    <div class="toolbar" style="justify-content:flex-end;">
-      <button id="summaryExportTableBtn" type="button" class="ghost">⬇️ Export Tabelle (wie angezeigt)</button>
-      <button id="summaryExportAllBtn" type="button" class="ghost">⬇️ Export Alles</button>
...
-
-    <div id="swapStep1">
-      <p>Schritt 1: Wähle die <b>Feldspieler</b>, die <b>raus</b> sollen (rot).</p>
...
-      <p>Schritt 2: Wähle die <b>Bankspieler</b>, die <b>rein</b> sollen (grün).<br>
-        Benötigt: <b><span id="swapNeeded">0</span></b> — Ausgewählt: <b><span id="swapHave">0</span></b></p>
-      <div id="swapBenchList" class="chips"></div>
-      <div class="sep"></div>
-      <div class="toolbar" style="justify-content:space-between;">
-        <button id="swapBack" type="button" class="ghost">Zurück</button>
...
-          <button id="swapConfirm" type="button" class="success" disabled>Bestätigen</button>
-        </div>
-      </div>
-    </div>
-  </div>
...
-    if (t.includes('pausiert') || t==='▶️ Weiter') return 'info';
-    return 'info';
-  }
...
-      minSpan.textContent = `${e.min ?? matchMinute()}’`;
...
-        outChip.textContent = `Raus: ${e.outName || '—'}`;
-        const inChip = document.createElement('span');
-        inChip.className = 'pair-in';
-        inChip.textContent = `Rein: ${e.inName || '—'}`;
...
-        if (parts.length){ details.textContent = parts.join(' • '); content.appendChild(details); }
-      }
-
...
-    log({ts: now(), type:'🔁 Wechsel', outName, inName, side:'us'});
-  }
...
-    if (!p) return;
-    if (p.squad !== 'out'){ alert("Du kannst nur Spieler löschen, die gerade grau sind."); return; }
-    if (!confirm(`"${p.name}" wirklich aus dem Kader löschen?`)) return;
...
-          <span class="stats">
-            <span class="stat">⚽ ${p.goals||0}</span>
-            <span class="stat">🅰️ ${p.assists||0}</span>
-          </span>
-        </div>
-      `;
...
-    log({ts: now(), type:'⏯️ Spiel gestartet'});
-  }
-
-  function pauseClock(){
-    if (!clock.running) return;
...
-    log({ts: now(), type:'⏸️ Spiel pausiert'});
-  }
-
...
-         </select>`
-      : `<p class="hint">Kein Mitspieler auf dem Feld. Wähle unten „Ohne Vorlage“.</p>`;
...
-    }
-
-    log({ts: now(), type:'⚽ Tor', name: scorer.name, assistName: assistName || undefined, side:'us'});
-    renderPlayers();
-  }
-
-  function addGoalFlow(){
-    if (!selection.playerId){ alert('Bitte zuerst den Torschützen markieren.'); return; }
-    const scorer = players.find(p=>p.id===selection.playerId);
-    if (!scorer || !scorer.onField){ alert('Torschütze muss auf dem Feld sein.'); return; }
-    promptAssistFor(scorer.id);
-  }
-
-  function ownGoal(){
-    scoreOpp += 1; $('scoreOpp').textContent = String(scoreOpp);
-    log({ts: now(), type:'⚠️ Gegentor', extra:'Gegner hat getroffen', side:'opp'});
-  }
-
-  /* Export */
-  function exportCSV(){ /* unverändert – bleibt hier, aber nur in Summary Buttons genutzt */
-    const lines = [];
...
-  function startMatch(){
-    const startCount = players.filter(p=>p.squad==='start').length;
-    if (startCount===0){ alert('Bitte mindestens einen Startspieler wählen.'); return; }
...
-    if (!confirm("Standard-Kader laden? (Überschreibt deinen gespeicherten Kader)")) return;
...
-    wrap.hidden = !open;
-    $('toggleEventsBtn').textContent = open ? "📋 Ausblenden" : "📋 Anzeigen";
...
-    $('assistModal').hidden = true;
-    if (assistId === selection.playerId){ alert('Torschütze und Assist dürfen nicht identisch sein.'); return; }
...
+      color:#fff;
+    ">
+      <h3 style="margin:0 0 14px 0;color:white;">🔒 Lizenz</h3>
+
+      <input id="licensePin"
+            type="password"
...
+          <div class="toolbar">
+            <button id="historyBtn" type="button" class="ghost">📊 Historie</button>
+          </div>
+        </div>
+
...
+
+        <div class="toolbar" style="margin-top:12px;">
+          <button id="startBtn" type="button" class="success" disabled>▶️ Start</button>
...
+          <span class="badge" id="matchOpponentBadge" hidden>vs —</span>
+        </div>
+
...
+
+        <div class="toolbar" style="margin-top:10px;justify-content:space-between;">
+          <button id="swapWizardBtn" type="button" class="ghost">🔁 Wechsel</button>
+          <div class="toolbar">
+            <button id="goalBtn" type="button" class="ghost">⚽ Tor</button>
+            <button id="ownGoalBtn" type="button" class="ghost danger">⚠️ Gegentor</button>
+            <button id="endBtn" type="button" class="danger">⏹️ Ende</button>
+          </div>
+        </div>
+
+        <div class="sep"></div>
...
+          <button id="toggleEventsBtn" type="button" class="ghost">📋</button>
+        </div>
+        <div id="eventsWrap" hidden style="margin-top:10px">
+          <div id="eventLog" class="timeline"></div>
+        </div>
...
+          <span class="badge" id="targetBadge">Ziel Ø: —</span>
+        </div>
+        <div class="list" id="playerList"></div>
+      </div>
+
...
+      <div class="sep"></div>
+      <div class="toolbar" style="justify-content:flex-end;">
+        <button id="summaryShareImgBtn" type="button" class="success">📲 WhatsApp Bild</button>
+        <button id="summaryExportTableBtn" type="button" class="ghost">⬇️ Export Tabelle</button>
+        <button id="summaryExportAllBtn" type="button" class="ghost">⬇️ Export Alles</button>
...
+        <button id="historyCloseBtn" type="button" class="secondary">Schließen</button>
+      </div>
+
+      <div class="sep"></div>
+
+      <!-- ✅ Gesamt über letzte 15 Spiele -->
...
+
+      <div class="toolbar" style="justify-content:flex-end;">
+        <button id="historyClearBtn" type="button" class="danger">🗑️ Löschen</button>
+      </div>
+    </div>
+  </div>
+
...
+        <button id="historyDetailCloseBtn" type="button" class="secondary">Schließen</button>
+      </div>
+
...
+
+      <div class="toolbar" style="justify-content:flex-end;">
+        <button id="historyDetailExportBtn" type="button" class="ghost">⬇️ Export</button>
+        <button id="historyDetailDeleteBtn" type="button" class="danger">🗑️ Löschen</button>
+      </div>
+    </div>
+  </div>
+
...
+    targetBadge.textContent = t > 0 ? `Ziel Ø: ${Math.round(t)} min` : `Ziel Ø: —`;
+  }
+
...
+    const p = players.find(x=>x.id===id);
+    if (!p) return;
+    if (p.squad !== 'out'){ alert("Nur graue Spieler löschbar."); return; }
...
+      const sub = targetMin>0 ? `Ø ${Math.round(targetMin)} min • ${Math.ceil(remaining)} min` : '';
+
+      card.innerHTML = `
+        <header>
+          <div class="name">${p.name}</div>
...
+
+        <div class="meta">
+          <span class="badge playtime-badge">⏱️ <b>${fmtTime(currentPlaytime(p))}</b></span>
+          <span class="stats">
+            <span class="stat">⚽ ${p.goals||0}</span>
+            <span class="stat">🅰️ ${p.assists||0}</span>
+          </span>
...
+      if (sub && targetMin>0){
+        const remaining = Math.max(0, targetMin - playedMin);
+        sub.textContent = `Ø ${Math.round(targetMin)} min • ${Math.ceil(remaining)} min`;
+      }
+    });
+    updateTargetBadge();
+  }
...
+    $('sumMetaLine').textContent = `Endstand: ${scoreUs}:${scoreOpp}` + (metaParts.length ? ` • ${metaParts.join(' • ')}` : "");
+
+    const tbody = $('summaryTable').querySelector('tbody');
+    tbody.innerHTML = '';
+
...
+           <option value="">— Ohne Assist —</option>
+           ${onField.map(p=>`<option value="${p.id}">${p.name}</option>`).join('')}
+         </select>`
...
+
+  function addGoalFlow(){
+    if (!selection.playerId){ alert('Torschütze markieren.'); return; }
+    const scorer = players.find(p=>p.id===selection.playerId);
+    if (!scorer || !scorer.onField){ alert('Torschütze muss auf dem Feld sein.'); return; }
+    promptAssistFor(scorer.id);
+  }
...
+  function ownGoal(){
+    scoreOpp += 1; $('scoreOpp').textContent = String(scoreOpp);
+    log({ts: now(), type:'⚠️ Gegentor', side:'opp'});
+  }
+
+  /* ===== Export CSV ===== */
...
+    });
+
+    // Canvas – dynamisch (14 Spieler passt locker)
+    const pad = 28;
+    const rowH = 48;
+    const headerH = 54;
+    const titleH = 64;
...
+  }
+
+  /* ===== Historie: Gesamt über 15 Spiele ===== */
...
+      const title = `${fmtDateShort(m.createdAt)} • ${opp}`;
+      const sub = `${m.totalMinutes || ''} min • ${m.onFieldMax || ''} auf Feld`;
...
+    $('histMeta').textContent = `${m.scoreUs}:${m.scoreOpp}` + (opp ? ` • ${opp}` : '') + (m.totalMinutes ? ` • ${m.totalMinutes} min` : '');
+
...
+  function startMatch(){
+    const startCount = players.filter(p=>p.squad==='start').length;
+    if (startCount===0){ alert('Startelf wählen.'); return; }
+    if (startCount>onFieldMax){ alert(`Max. ${onFieldMax} Startspieler.`); return; }
+
...
+    $('assistModal').hidden = true;
+    if (assistId === selection.playerId){ alert('Assist ≠ Torschütze.'); return; }
+    recordGoalWithAssist(selection.playerId, assistId);
+  });
+
+  // Summary modal
...
+    if (!confirm('Historie löschen?')) return;
+    clearHistory();
+    renderHistoryTotals();
+    renderHistoryList();
+  });
